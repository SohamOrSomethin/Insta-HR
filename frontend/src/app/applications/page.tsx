      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Interviews', value: applications.filter(a => a.status === 'interview_scheduled').length },
        ...
        {applications.length === 0 ? (
          <div className="text-center py-20">
            ...
            <Link href="/jobs" className="px-6 py-3 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition-colors">Browse Jobs</Link>
          </div>
        ) : (
          <div className="space-y-4">
            {applications.map(app => {
              const status = STATUS_CONFIG[app.status] || { label: app.status, color: 'bg-gray-100 text-gray-700' }
              return (
                <div key={app.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="text-lg font-bold text-gray-900 mb-1">{app.Job?.title ?? 'Job Title'}</h3>
                      <p className="text-gray-500 text-sm">
                        {app.Job?.location ?? 'Location'} • {app.Job?.industry ?? 'Industry'} • {app.Job?.jobType ?? 'Type'}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link href={`/jobs/${app.jobId}`} className="px-4 py-2 bg-blue-50 text-blue-600 text-sm font-medium rounded-xl hover:bg-blue-100 transition-colors">View Job</Link>
                  </div>
                  {app.interviewDate && (
                    <div className="mt-3 bg-purple-50 rounded-xl p-3">
                      <p className="text-purple-700 text-sm font-medium">📅 Interview: {new Date(app.interviewDate).toLocaleString()}</p>
                    </div>
                  )}
                </div>
              )
