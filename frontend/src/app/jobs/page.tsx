...
export default function JobsPage() {
  ...
  const toggleSave = async (e: React.MouseEvent, jobId: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v1/jobs-actions/save/${jobId}`, {
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) { setJobs(data.data); setTotal(data.total) }
    } catch { console.error('Failed to fetch jobs') }
    setLoading(false)
  }

  useEffect(() => { fetchJobs(); fetchSavedJobIds() }, [])

  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); fetchJobs() }

  const clearFilters = () => {
    setKeyword(''); setLocation(''); setIndustry(''); setJobType('')
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 py-12 px-6">
        ...
        <form onSubmit={handleSearch} className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl p-4 shadow-lg flex flex-wrap gap-3">
            <input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="Job title, skills, keywords..."
            ...
            <button type="submit" className="px-6 py-2 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition-colors text-sm">Search</button>
          </div>
        </form>
      </div>
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          {(keyword || location || industry || jobType) && (
            <button onClick={clearFilters} className="text-blue-600 text-sm hover:underline">Clear filters</button>
          )}
        </div>
        {loading ? (
