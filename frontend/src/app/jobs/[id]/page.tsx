...
export default function JobDetailPage() {
  ...
  const handleApply = async () => {
    ...
    setApplying(true)
    try {
      const res = await fetch(`${API_URL}/api/v1/applications/apply/${id}`, {
        method: 'POST',
  ...
  if (!job) return (
    <main className="min-h-screen bg-gray-50 pt-24 flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/jobs" className="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700">Back to Jobs</Link>
      </div>
    </main>
  )
  ...
  return (
    <main className="min-h-screen bg-gray-50 pt-16">
      <div className="max-w-4xl mx-auto px-6 py-10">
        <Link href="/jobs" className="text-blue-600 hover:underline text-sm mb-6 inline-block">← Back to Jobs</Link>
        {message && (
        ...
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 mb-6">
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-green-600 font-bold text-lg">₹{Number(job.salaryMin).toLocaleString()} – ₹{Number(job.salaryMax).toLocaleString()}</p>
          ...
          {userRole === 'candidate' && !message && (
            <div>
              {!showApplyBox ? (
                <button onClick={() => setShowApplyBox(true)} className="px-8 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors">Apply Now</button>
              ) : (
                <div className="mt-4 space-y-3">
                  <textarea value={coverLetter} onChange={e => setCoverLetter(e.target.value)}
                    placeholder="Write a short cover letter (optional)..." rows={4}
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => setShowApplyBox(false)}
                      className="px-6 py-3 bg-gray-100 text-gray-600 font-medium rounded-xl hover:bg-gray-200 transition-colors">Cancel</button>
                  </div>
                </div>
              )}
            </div>
          ...
          {!userRole && (
            <Link href="/login" className="px-8 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors inline-block">Login to Apply</Link>
          )}
        </div>
